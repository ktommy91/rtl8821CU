...
chbw_decision:
	ifbmp_ch_changed = rtw_ap_chbw_decision(padapter, parm->ifbmp, req_ch, req_bw, req_offset
	...
	rtw_dfs_master_status_apply(padapter, mlme_act);
#endif
...
u8 rtw_ap_chbw_decision(_adapter *adapter, u8 ifbmp, s16 req_ch, s8 req_bw, s8 req_offset
			  , u8 *ch, u8 *bw, u8 *offset, u8 *chbw_allow)
{
	...
	struct rf_ctl_t *rfctl = adapter_to_rfctl(adapter);
	bool ch_avail = _FALSE;
	u8 cur_ie_ch[CONFIG_IFACE_NUMBER];
	u8 cur_ie_bw[CONFIG_IFACE_NUMBER];
	u8 cur_ie_offset[CONFIG_IFACE_NUMBER];
	u8 dec_ch[CONFIG_IFACE_NUMBER];
	u8 dec_bw[CONFIG_IFACE_NUMBER];
	u8 dec_offset[CONFIG_IFACE_NUMBER];
	...
	RTW_INFO("%s ifbmp:0x%02x req:%d,%d,%d\n", __func__, ifbmp, req_ch, req_bw, req_offset);
	...
	if (MSTATE_STA_LG_NUM(&mstate_others) && !MSTATE_STA_LD_NUM(&mstate_others)) {
		...
		*chbw_allow = _TRUE;
		...
		if (rtw_ap_ch_specific_chk(adapter, u_ch, &u_bw, &u_offset, __func__) == _FAIL) {
			...
			goto choose_chbw;
		} else if (rtw_chset_is_ch_non_ocp(chset, u_ch, u_bw, u_offset)) {
